...
$query="select * from pre_medical_history where id = '$NID_no' limit 1";	
		...
		if(!empty($info))
			{
				header("Location: update_medical_records.php");
			}
		}
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		$query = "insert into pre_medical_history (current_medicine,vitamin,allergies,major_surgery_details,minor_surgery_details,major_disease_details,minor_disease_details,any_bad_habit,any_accidental_case,id) 
		values ('$current_medicine','$vitamin','$allergies','$major_surgery_details','$minor_surgery_details','$major_disease_details','$minor_disease_details','$any_bad_habit','$any_accidental_case','$NID_no')";

			mysqli_query($con, $query);
			
			
		$query = "UPDATE patient SET history_id ='$NID_no'WHERE NID_no='$NID_no'";
...
<body>
	...
	#box{
		...
		<div id="box"><div style="font-size: 20px;margin: 10px;color: black;">Are you currently Taking Medicine(including over the counter and herbal medication): </div><input id="text" type="text" name="current_medicine"><br><br></div></div>
